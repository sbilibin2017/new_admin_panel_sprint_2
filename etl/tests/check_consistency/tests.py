...
import psycopg2
from db.settings import DSL, TABLES
...
def test_table_values() -> None:
    ...
    def get_sqlite_select_all(sqlite_cur: RealDictCursor, table_name: str) -> RealDictCursor:
        ...
        '''
        return sqlite_cur.execute(
            f"""SELECT * FROM {table_name} ORDER BY {table_name}.id""")
        ...
        for table_name in TABLES:
            ...
            while True:
                ...
                if sqlite_values:
                    ...
                    with pg_conn.cursor() as pg_cur:
                        psql_values = get_psql_values_in_chunk_with_idxs(
                            pg_cur, table_name, idxs)
